...
limni$nivel[limni$nivel <= 0] <- NA
limni <- na.omit(limni)

limni <- limni[c(2,5,7,8,9,10)]
...
max_f <- limni %>% 
  group_by(muniCodi, territoriales, fuente) %>% 
  summarise(maxf = max(as.Date(fecha, "%Y-%m-d HH:mm")))
...
report_datos <-  difftime(max(lluvia$fecha), min(lluvia$fecha), 
                              units = c("mins"))/5

report_datos <- as.numeric(report_datos)

lluvia_noerror <- lluvia
lluvia_noerror$muestra[lluvia_noerror$muestra < 0] <- NA
lluvia_noerror <- na.omit(lluvia_noerror)

#Agrupamos por municipios y hallamos su acumulado para la temporalidad dada
lluvia2 <-  lluvia_noerror %>%
...
comenzar <-  as.character(params$fecha[1])
finalizar <-  as.character(params$fecha[2] + 1)
...
limni <- limni[c(2,3,4,7,8,9,10)]

limni$caudal[limni$caudal == -999] <- NA
...
El número de estaciones `r parametro2` que reportaron datos en las fechas escogidas fue de: `r n_estaciones`.

El número de estaciones `r parametro2` que actualmente posee el programa, para las territoriales escogidas, es: `r n_total`.
...
if(params$n == 2) {
  kbl(limni_r, booktabs = T, align = "c",
      caption = "Resumen estaciones limnigráficas analizadas analizadas.") %>%
  ...
  kbl(limni_r, booktabs = T, align = "c",
      caption = "Resumen estaciones limnigráficas analizadas.") %>%
  kable_styling(latex_options =c("striped", "scale_down")) %>% 
  kable_styling(position = "center")%>%
  kable_styling(latex_options = "hold_position")
}
